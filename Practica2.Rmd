...
Identificación de Patrones Ocultos: Permite descubrir grupos de individuos con características similares (por ejemplo, patrones en ocupaciones, horas trabajadas y nivel de educación). Reducción de la Complejidad: Ayuda a simplificar la estructura de los datos, especialmente si se busca segmentar la población para análisis adicionales. Datos Mixtos: Las variables numéricas (age, hours_per_week) se pueden utilizar para el clustering, aunque puede ser necesario estandarizar los datos para evitar que las variables con mayor rango dominen el análisis.
...
cat("Número de núcleos utilizados:", num_cores, "\n")

# --- 4. Método Supervisado: Árbol de Decisión (Paralelizado) ---
set.seed(123)
...
# Entrenar el modelo en paralelo
income_model <- foreach(i = 1:num_cores, .combine = c, .packages = 'rpart') %dopar% {
  rpart(income ~ age + workclass + education + marital_status + occupation + relationship + race + sex + capital_gain + capital_loss + hours_per_week + native_country,
        data = train_data, method = "class")
}

# Evaluar el modelo
predictions <- predict(income_model[[1]], test_data, type = "class")
conf_matrix <- confusionMatrix(predictions, test_data$income)
print(conf_matrix)

# --- 5. Método No Supervisado: K-Means Clustering (Paralelizado) ---
...
# Aplicar K-Means en paralelo
set.seed(123)
kmeans_result <- foreach(i = 1:num_cores, .combine = c, .packages = 'cluster') %dopar% {
  kmeans(numeric_data, centers = 3)
}

# Mostrar resultados
table(kmeans_result[[1]]$cluster)
